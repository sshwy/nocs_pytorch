...
def load_mask(dataset_dict, subset):
    ...
    if dataset_dict["source"] in ["CAMERA", "Real"]:
        ...
        masks, coords, class_ids, scales = process_data(
...
def mapper(dataset_dict):
    ...
    mask, coord, class_ids, scales, domain_label = load_mask(
...
if __name__ == "__main__":
    ...
    if mode == "detect":
        ...
        cfg.MODEL.WEIGHTS = os.path.join(
            "/data2/qiweili/logs/nocs_bin4/step3", "model_final.pth"
        )  # 我们进行训练的 权值文件  存放处
            ...
            for i, image_dict in enumerate(data_dict):
                ...
                if 0:
                    ...
                    if not data in ["coco_val", "coco_train"]:
                        ...
                        start = time.time()
                        result["gt_RTs"], _, error_message, _ = utilscopy.align(
                            gt_class_ids,
                            gt_mask,
                            gt_coord,
                            depth,
                ...
                save_path = os.path.join(
                    save_dir, "results_{}.pkl".format(image_short_path)
                )
                with open(save_path, "wb") as f:
                    cPickle.dump(result, f)
                print(
                    "Results of image {} has been saved to {}.".format(
