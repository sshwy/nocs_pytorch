...
def load_mask(dataset_dict):
    ...
    if dataset_dict["source"] in ["CAMERA", "Real"]:
        ...
        masks, coords, class_ids, scales = process_data(
            mask_im, coord_map, dataset_dict["inst_dict"], meta_path
        )
        ...
        for annotation in annotations:
            ...
            if class_id:
                m = utilscopy.annToMask(
                    annotation, dataset_dict["height"], dataset_dict["width"]
                )
...
def mapper(dataset_dict):
    ...
    aug = True
    if aug:
        image_after, mask, coord, class_ids, scales, domain_label = aug_load_mask(
...
class mydataloader:
    def __init__(self, cfg):
        ...
        self.loader.append(
            iter(build_detection_train_loader(cfg, mapper=mapper, dataset=dataset))
        )
    ...
    def __next__(self):  # __next__用于返回下一个，返回下一个才能被称之为迭代器。
        ...
        assert np.sum(weight) == 1, "[ Error ]: total sum of weights is {} != 1".format(
            np.sum(weight)
        )
