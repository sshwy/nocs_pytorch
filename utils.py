...
def compute_iou_mAP(final_results, synset_names, iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
...
def draw_detections(image, save_dir, data_name, image_id, intrinsics, synset_names, draw_rgb_coord,
                    ...
                    draw_gt=True, draw_pred=True, draw_tag=True):
        ...
        if draw_tag:
            if gt_class_ids is not None: ## if gt exists, draw rotation and translation error
                ...
                for ind in range(num_pred_instances):
                    ...
                    degree, cm = compute_RT_degree_cm_symmetry(RT, gt_RT, cls_id, gt_handle_visibility[i], synset_names)
...
def R_c_computation(K_c,c,fushu=0):
    ...
    R_c = I + r_x + torch.bmm(r_x,r_x) /(torch.ones([batch_size,1,1])+ (p*q).sum(1,keepdim = True))
...
def from_out_unmold_coord(r):
    ...
    for i in range(inst_num) :
        im_coord = unmold_coord(mrcnn_coords[i,:,:,:],list(r.pred_boxes.tensor.int().cpu().numpy()[i]),r.image_size)
        '''
        a = im_coord
        a=a*255
        d = a.astype(int)
