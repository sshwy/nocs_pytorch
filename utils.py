...
def compute_mean_l1_coord_diff(mask1, mask2, coord1, coord2, synset, cls_id):
    ...
    mask1 = np.reshape(mask1 > 0.5, (-1)).astype(np.float32)
    mask2 = np.reshape(mask2 > 0.5, (-1)).astype(np.float32)
    ...
    if synset[cls_id] in ["bottle", "bowl", "can"]:
        ...
        for i in range(M):
            rotated_pts1 = (
                rotation_y_matrix(float(i) * np.float32(2 * math.pi / M)) @ pts1
            )
            pts1_symmetry[:, :, i] = rotated_pts1

        pts2_reshape = pts2.reshape([3, -1, 1])
        mean_dists = np.mean(
            np.linalg.norm(pts1_symmetry - pts2_reshape, axis=0), axis=0
        )
        ...
        for i in range(2):
            rotated_pts1 = (
                rotation_y_matrix(float(i) * np.float32(2 * math.pi / 2)) @ pts1
            )
            # print(rotated_pts1)
            pts1_symmetry[:, :, i] = rotated_pts1
...
def compute_RT_projection_2d_symmetry(
    RT_1,
    RT_2,
    class_id,
    handle_visibility,
    ...
    if (synset_names[class_id] in ["bottle", "can", "bowl"]) or (
        ...
        for i in range(1, num_rotation):
            ...
            coords_3d_2 = transform_coordinates_3d(
                y_rotation_matrix(theta) @ vertices, RT_2
            )
            projected_2 = calculate_2d_projections(coords_3d_2, intrinsics)
...
def rotate_image(image, new_w, new_h, affine_mat, interpolation=cv2.INTER_LINEAR):
    ...
    # Apply the transform
    result = cv2.warpAffine(image, affine_mat, (new_w, new_h), flags=interpolation)
...
def rotate_and_crop(image, rotate_degree, interpolation):
    ...
    image_rotated_cropped = crop_around_center(
        image_rotated,
        *largest_rotated_rect(image_width, image_height, math.radians(rotate_degree))
    )
...
def rotate_and_crop_images(image, masks, coords, rotate_degree):
    ...
    rect = largest_rotated_rect(image_width, image_height, math.radians(rotate_degree))

    image_rotated_cropped = crop_around_center(image_rotated, *rect)
    mask_rotated_cropped = crop_around_center(mask_rotated, *rect)

    image_rotated_cropped = cv2.resize(
        image_rotated_cropped,
        (image_width, image_height),
        interpolation=cv2.INTER_LINEAR,
    )
    mask_rotated_cropped = cv2.resize(
        mask_rotated_cropped,
        (image_width, image_height),
        interpolation=cv2.INTER_NEAREST,
    )

    if coords is not None:
        coord_rotated = rotate_image(
            coords, new_w, new_h, affine_mat, cv2.INTER_NEAREST
        )
        coord_rotated_cropped = crop_around_center(coord_rotated, *rect)
        coord_rotated_cropped = cv2.resize(
            coord_rotated_cropped,
            (image_width, image_height),
            interpolation=cv2.INTER_NEAREST,
        )
...
class Dataset(object):
    ...
    def prepare(self, class_map=None):
        ...
        # ['BG', 'bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']中的一个，顺序与class_ids对应
        self.class_names = []
        ...
        for cls_info in self.class_info:
            ...
            if source == "coco":
                ...
                self.class_from_source_map[map_key] = self.class_names.index(
                    class_map[cls_info["name"]]
                )
            else:
    ...
    def map_source_class_id(self, source_class_id):
        ...
        return (
            self.class_from_source_map[source_class_id]
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    # for coordinate map
        ...
        for i in range(mask.shape[-2]):
            ...
            y1, x1, y2, x2 = bbox[i][:4]
            m = m[y1:y2, x1:x2, :] * 255
            m = scipy.misc.imresize(
                m.astype(np.uint8), mini_shape + (mask.shape[-1],), interp="nearest"
            )
...
def expand_mask(bbox, mini_mask, image_shape):
    ...
    if len(mini_mask.shape) == 3:
        ...
        for i in range(mask.shape[-2]):
            ...
            w = x2 - x1
            m = scipy.misc.imresize(
                m.astype(float), (h, w, mini_mask.shape[-1]), interp="nearest"
            )
...
def compute_matches(
    ...
    if len(gt_class_ids) == 0 or len(pred_class_ids) == 0:
        return (
            -1 * np.ones([len(gt_class_ids)]),
            -1 * np.ones([len(pred_class_ids)]),
            None,
            np.zeros([0]),
        )
...
def compute_RT_projection_2d_overlaps(
    gt_class_ids,
    gt_RTs,
    gt_handle_visibility,
    pred_class_ids,
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_RT_projection_2d_symmetry(
                pred_RTs[i],
                gt_RTs[j],
                gt_class_ids[j],
                gt_handle_visibility[j],
...
def compute_match_from_projection_2d_dist(
    overlaps, pred_class_ids, gt_class_ids, projection_thres_list
):
...
def compute_degree_cm_mAP(
    ...
    iou_output_path = os.path.join(
        log_dir, "IoU_3D_AP_{}-{}.png".format(iou_thres_list[0], iou_thres_list[-1])
    )
    iou_dict_pkl_path = os.path.join(
        log_dir, "IoU_3D_AP_{}-{}.pkl".format(iou_thres_list[0], iou_thres_list[-1])
    )
    ...
    pose_dict_pkl_path = os.path.join(
        log_dir,
        prefix
        + "AP_{}-{}degree_{}-{}cm.pkl".format(
            degree_thres_list[0],
            degree_thres_list[-2],
            shift_thres_list[0],
            shift_thres_list[-2],
    ...
    output_path = os.path.join(
        log_dir,
        prefix
        + "mAP_{}-{}degree.png".format(degree_thres_list[0], degree_thres_list[-2]),
    )
    ...
    print(
        "5 degree, 100cm: {:.1f}".format(
            pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(100)] * 100
        )
    )
    print(
    ...
    print(
        "15 degree, 5cm: {:.1f}".format(
            pose_aps[-1, degree_thres_list.index(15), shift_thres_list.index(5)] * 100
        )
    )
    print(
        "15 degree, 10cm: {:.1f}".format(
            pose_aps[-1, degree_thres_list.index(15), shift_thres_list.index(10)] * 100
        )
    )
...
def compute_2D_projection_mAP(
    ...
    projection_pred_matches_all = [
        np.zeros((num_projection_thres, 0)) for _ in range(num_classes)
    ]
    projection_pred_scores_all = [
        np.zeros((num_projection_thres, 0)) for _ in range(num_classes)
    ]
    projection_gt_matches_all = [
        np.zeros((num_projection_thres, 0)) for _ in range(num_classes)
    ]
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_projection_2d_overlaps = compute_RT_projection_2d_overlaps(
                cls_gt_class_ids,
                cls_gt_RTs,
                cls_gt_handle_visibility,
                cls_pred_class_ids,
            ...
            (
                projection_cls_gt_match,
                projection_cls_pred_match,
            ) = compute_match_from_projection_2d_dist(
                RT_projection_2d_overlaps,
                cls_pred_class_ids,
                cls_gt_class_ids,
                projection_thres_list,
            )

            projection_pred_matches_all[cls_id] = np.concatenate(
                (projection_pred_matches_all[cls_id], projection_cls_pred_match),
                axis=-1,
            )

            cls_pred_scores_tile = np.tile(cls_pred_scores, (num_projection_thres, 1))
            projection_pred_scores_all[cls_id] = np.concatenate(
                (projection_pred_scores_all[cls_id], cls_pred_scores_tile), axis=-1
            )
            assert (
                projection_pred_scores_all[cls_id].shape[1]
                == projection_pred_matches_all[cls_id].shape[1]
            ), "{} vs. {}".format(
                projection_pred_scores_all[cls_id].shape,
                projection_pred_matches_all[cls_id].shape,
            )
            projection_gt_matches_all[cls_id] = np.concatenate(
                (projection_gt_matches_all[cls_id], projection_cls_gt_match), axis=-1
            )
    ...
    projection_dict_pkl_path = os.path.join(
        log_dir,
        prefix
        + "Projection_2D_AP_{}-{}pixel.pkl".format(
            projection_thres_list[0], projection_thres_list[-1]
        ),
    )
    pose_dict = {}
    ...
    for i, projection_thres in enumerate(projection_thres_list):
        for cls_id in range(1, num_classes):
            projection_2d_aps[cls_id, i] = compute_ap_from_matches_scores(
                projection_pred_matches_all[cls_id][i, :],
    ...
    for i, cls_id in enumerate([1, 3, 4, 5, 6, 7, 8, 9]):
        ...
        for curve_path in curve_data:
            ...
            axarr[i].plot(
                curve[:, 0], curve[:, 1], label=curve_name, linewidth=LINE_WIDTH
            )
    ...
    for cls_id in range(1, num_classes):
        ...
        print(class_name)
        ax_pose.plot(
            projection_thres_list, projection_2d_aps[cls_id, :], label=class_name
        )
    ...
    output_path = os.path.join(
        log_dir,
        prefix
        + "Projection_2D_AP_{}-{}pixel.png".format(
            projection_thres_list[0], projection_thres_list[-1]
        ),
    )
    fig_pose.legend()
    ...
    print(
        "3D IoU at 25: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.25)] * 100)
    )
    print(
        "3D IoU at 50: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.5)] * 100)
    )
    print(
        "2D Projection at 2.5 pixel: {:.1f}".format(
            projection_2d_aps[-1, projection_thres_list.index(2.5)] * 100
        )
    )
    print(
        "2D Projection at 5 pixel: {:.1f}".format(
            projection_2d_aps[-1, projection_thres_list.index(5)] * 100
        )
    )
    print(
        "2D Projection at 10 pixel: {:.1f}".format(
            projection_2d_aps[-1, projection_thres_list.index(10)] * 100
        )
    )
    print(
        "2D Projection at 15 pixel: {:.1f}".format(
            projection_2d_aps[-1, projection_thres_list.index(15)] * 100
        )
    )
    print(
        "2D Projection at 20 pixel: {:.1f}".format(
            projection_2d_aps[-1, projection_thres_list.index(20)] * 100
        )
    )
...
def compute_coords_aps(final_results, synset_names, iou_thresholds, coord_thresholds):
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            gt_match, pred_match, overlaps, pred_indices = compute_matches(
                cls_gt_bboxes,
                cls_gt_class_ids,
                cls_gt_masks,
                cls_pred_bboxes,
            ...
            if len(gt_match) and len(pred_match):
                ...
                for i in range(len(pred_match)):
                    if pred_match[i] > -1:
                        j = int(pred_match[i])
                        mean_coord_dist = compute_mean_l1_coord_diff(
                            cls_pred_masks_sorted[..., i],
                            cls_gt_masks[..., j],
    ...
    for cls_id in range(1, num_classes):
        ...
        print(
            "mean coord dist of {} class: {}".format(
                synset_names[cls_id], np.mean(mean_coord_dist_cls[cls_id])
            )
        )


def compute_3d_matches_degree_cm(
    gt_class_ids,
    gt_RTs,
    gt_scales,
    pred_boxes,
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_RT_upper_bound_symmetry(
                pred_RTs[i],
                gt_RTs[j],
                gt_class_ids[j],
                synset_names,
...
def load_mesh(mesh_path, is_save=False, is_normalized=False, is_flipped=False):
    ...
    for l in lines:
        ...
        if words[0] == "f":
            ...
            faces.append(
                [int(face_words[1]) - 1, int(face_words[2]) - 1, int(face_words[3]) - 1]
            )
...
def draw_coord_mask(
    image, class_ids, RTs, Vs, Fs, intrinsics, output_path, is_overlay=True
):
...
def draw_text(draw_image, bbox, text, draw_box=False):
    ...
    text_box_pos_tl = (
        min(bbox[1] + bbox_margin, 635 - retval[0] - 2 * text_margin),
        min(bbox[2] + bbox_margin, 475 - retval[1] - 2 * text_margin),
    )
    text_box_pos_br = (
        text_box_pos_tl[0] + retval[0] + 2 * text_margin,
        text_box_pos_tl[1] + retval[1] + 2 * text_margin,
    )
    ...
    if draw_box:
        ...
        cv2.rectangle(
            draw_image, (bbox[1], bbox[0]), (bbox[3], bbox[2]), (255, 0, 0), 2
        )

    cv2.rectangle(draw_image, text_box_pos_tl, text_box_pos_br, (255, 0, 0), -1)

    cv2.rectangle(draw_image, text_box_pos_tl, text_box_pos_br, (0, 0, 0), 1)

    cv2.putText(
        draw_image, text, text_pos, fontFace, fontScale, (255, 255, 255), thickness
    )
...
def draw_detections(
    ...
    if draw_gt:
        output_path = os.path.join(
            save_dir, "{}_{}_coord_gt.png".format(data_name, image_id)
        )
        ...
        for i in range(num_gt_instances):
            ...
            coord_data[:, :, 2] = (
                1 - coord_data[:, :, 2]
            )  # undo the z axis flipping to match original data
        ...
        if draw_tag:
            for i in range(num_gt_instances):
                ...
                overlay = draw_text(
                    overlay, gt_bbox[i], synset_names[gt_class_ids[i]], draw_box=True
                )
        ...
        output_path = os.path.join(
            save_dir, "{}_{}_bbox_gt.png".format(data_name, image_id)
        )
        ...
        if gt_RTs is not None:
            for ind, RT in enumerate(gt_RTs):
                ...
                xyz_axis = (
                    0.3
                    * np.array([[0, 0, 0], [0, 0, 1], [0, 1, 0], [1, 0, 0]]).transpose()
                )
                ...
                projected_bbox = calculate_2d_projections(
                    transformed_bbox_3d, intrinsics
                )
                draw_image_bbox = draw(
                    draw_image_bbox, projected_bbox, projected_axes, (255, 0, 0)
                )
    ...
    if draw_pred:
        ...
        output_path = os.path.join(
            save_dir, "{}_{}_coord_pred.png".format(data_name, image_id)
        )
        output_path_r = os.path.join(
            save_dir, "{}_{}_coord_pred_r.png".format(data_name, image_id)
        )
        output_path_g = os.path.join(
            save_dir, "{}_{}_coord_pred_g.png".format(data_name, image_id)
        )
        output_path_b = os.path.join(
            save_dir, "{}_{}_coord_pred_b.png".format(data_name, image_id)
        )
        ...
        if draw_tag:
            for i in range(num_pred_instances):
                overlay = draw_image.copy()
                text = synset_names[pred_class_ids[i]] + "({:.2f})".format(
                    pred_scores[i]
                )
        ...
        output_path = os.path.join(
            save_dir, "{}_{}_bbox_pred.png".format(data_name, image_id)
        )
        draw_image_bbox = image.copy()
        ...
        for ind in range(num_pred_instances):
            ...
            if (
                gt_class_ids is not None
            ):  ## if gt exists, skip instances that fail to match
        ...
        if draw_tag:
            if (
                gt_class_ids is not None
            ):  ## if gt exists, draw rotation and translation error
                i = 0
                for ind in range(num_pred_instances):
                    gt_ind = int(pred_match[ind])
                    if gt_ind == -1:
                    ...
                    degree, cm = compute_RT_degree_cm_symmetry(
                        RT, gt_RT, cls_id, gt_handle_visibility[i], synset_names
                    )
                    text = "{}({:.1f}, {:.1f})".format(synset_names[cls_id], degree, cm)
                    overlay = draw_text(overlay, pred_bbox[ind], text)
                    cv2.addWeighted(
                        overlay, alpha, draw_image_bbox, 1 - alpha, 0, draw_image_bbox
                    )
...
def draw_coco_detections(
    ...
    output_path = os.path.join(
        save_dir, "{}_{:04d}_image.png".format(data_name, image_id)
    )
    cv2.imwrite(output_path, image[:, :, ::-1])

    output_path = os.path.join(
        save_dir, "{}_{:04d}_mask_gt.png".format(data_name, image_id)
    )
    draw_image = image.copy()
    for i in range(len(gt_class_ids)):
        ...
        draw_image = draw_text(
            draw_image, gt_bbox[i], synset_names[gt_class_ids[i]], draw_box=True
        )
    cv2.imwrite(output_path, draw_image[:, :, ::-1])

    # Vs, Fs = dataset.load_objs(image_id, is_normalized=True) ## scale is estimated in RT
    output_path = os.path.join(
        save_dir, "{}_{:04d}_coord_pred.png".format(data_name, image_id)
    )
    output_path_r = os.path.join(
        save_dir, "{}_{:04d}_coord_pred_r.png".format(data_name, image_id)
    )
    output_path_g = os.path.join(
        save_dir, "{}_{:04d}_coord_pred_g.png".format(data_name, image_id)
    )
    output_path_b = os.path.join(
        save_dir, "{}_{:04d}_coord_pred_b.png".format(data_name, image_id)
    )
    # utils.draw_coord_mask(image, r['class_ids'], pred_RTs, Vs, Fs, intrinsics, output_path)
    draw_image = image.copy()
    if draw_rgb_coord:
    ...
    for i in range(len(pred_class_ids)):
        ...
        draw_image[cind, rind] = coord_data[cind, rind] * 255
        draw_image = draw_text(
            draw_image, pred_bbox[i], synset_names[pred_class_ids[i]], draw_box=True
        )
        if draw_rgb_coord:
            b_image[cind, rind, 2] = coord_data[cind, rind, 2] * 255
            b_image[cind, rind, 0:2] = 0
...
def backproject(depth, intrinsics, instance_mask):
    ...
    grid = np.array([idxs[1], idxs[0]])  # (2,  num_pixel)
...
def norm(t):
    sum = torch.sum(torch.pow(t, 2), dim=1)
    r = t / torch.sqrt(sum.unsqueeze(1))
    return r


def R_c_computation(K_c, c, fushu=0):
    """
    ...
    """
    c = c
    K_c = K_c.unsqueeze(0).repeat([c.shape[0], 1, 1])
    batch_size = c.shape[0]
    c = torch.cat((c, torch.ones(batch_size, 1)), dim=1).unsqueeze(2).float()
    ...
    if fushu == 2:
        q[:, 0], q[:, 1] = q[:, 1], q[:, 0]
    q = norm(q)
    p = torch.eye(3)[-1].unsqueeze(0).repeat([batch_size, 1, 1]).permute(0, 2, 1)

    r = torch.cross(p, q).squeeze(2)
    ...
    I = torch.eye(3).unsqueeze(0).repeat([batch_size, 1, 1]).permute(0, 2, 1)
    R_c = (
        I
        + r_x
        + torch.bmm(r_x, r_x)
        / (torch.ones([batch_size, 1, 1]) + (p * q).sum(1, keepdim=True))
    )
    # print('right',torch.matmul (R_c,q))
    # input()

    return torch.inverse(R_c[0]).numpy()
...
def align(
    ...
    for i in range(num_instances):
        ...
        b = b*255
        b = b.astype(int)
        cv2.imwrite('mask_c.jpg',b)
        """
        coord = coords[:, :, i, :]
        abs_coord_pts = np.abs(coord[mask == 1] - 0.5)  # (N * 3)
        ...
        try:
            ...
            scales, rotation, translation, outtransform = estimateSimilarityTransform(
                coord_pts, pts, False
            )
            # exit(0)
            ...
            if save_path is not None:
                coord_pts_rotated = (
                    aligned_RT[:3, :3] @ coord_pts.transpose() + aligned_RT[:3, 3:]
                )
                coord_pts_rotated = coord_pts_rotated.transpose()
                np.savetxt(
                    save_path + "_{}_{}_depth_pts.txt".format(i, class_name), pts
                )
                np.savetxt(
                    save_path + "_{}_{}_coord_pts.txt".format(i, class_name), coord_pts
                )
                np.savetxt(
                    save_path + "_{}_{}_coord_pts_aligned.txt".format(i, class_name),
                    coord_pts_rotated,
                )
...
def align_ICP(
    ...
    for i in range(num_instances):
        ...
        glob_paths = (
            "/home/hewang/Projects/CoordRCNN/data/pts/real_test/{}*.txt".format(
                synset_names[class_ids[i]]
            )
        )
        print(glob_paths)
        ...
        # try:
        if True:
            scales, rotation, translation = ICP.doICP(
                shape_pts, depth_pts, threshold=5, isViz=False
            )
...
def unmold_coord(coord, bbox, image_shape):
    ...
    # Put the mask in the right location.
    full_coord = np.zeros((image_shape[0], image_shape[1], 3), dtype=np.float32)
    full_coord[y1:y2, x1:x2, :] = coord
    return full_coord


def from_out_unmold_coord(r):
    inst_num = r.pred_coord.shape[0]
    if inst_num == 0:
        return np.array([])
    mrcnn_coords = r.pred_coord
    mrcnn_coords = mrcnn_coords.cpu().numpy()
    im_coords = []
    for i in range(inst_num):
        im_coord = unmold_coord(
            mrcnn_coords[i, :, :, :],
            list(r.pred_boxes.tensor.int().cpu().numpy()[i]),
            r.image_size,
        )
        """
        a = im_coord
        ...
        """
        im_coords.append(im_coord)
    return np.array(im_coords).transpose(1, 2, 0, 3)


def prase_instance(instance):
    pred_coords = from_out_unmold_coord(instance)  # [num_instance, 480, 640, 3]
    r = {}
    r["class_ids"] = instance.pred_classes.int().cpu().numpy() + 1
    r["scores"] = instance.scores.cpu().numpy()
    r["masks"] = instance.pred_masks.cpu().numpy().transpose(1, 2, 0)
    r["coords"] = pred_coords
    temp_roi = instance.pred_boxes.tensor.int().cpu().numpy()
    if len(r["class_ids"]) == 0:
        r["rois"] = []
    else:
        r["rois"] = np.empty(temp_roi.shape, dtype=int)
        r["rois"][:, 0] = temp_roi[:, 1]
        r["rois"][:, 1] = temp_roi[:, 0]
        r["rois"][:, 2] = temp_roi[:, 3]
        r["rois"][:, 3] = temp_roi[:, 2]
