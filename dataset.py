...
class NOCSDataset(utilscopy.Dataset):
    ...
    def __init__(self, synset_names, subset, config=Config()):
        ...
        config.OBJ_MODEL_DIR = os.path.join("/data2", "qiweili", "data", "obj_models")
        self.config = config
    ...
    def load_camera_scenes(self, dataset_dir, if_calculate_mean=False):
        ...
        image_ids = range(int(num_total_folders / 100))
        image_ids = range(1)
        ...
        for i in image_ids:
            ...
            meta_path = os.path.join(
                image_dir,
                "{:05d}".format(folder_id),
                "{:04d}_meta.txt".format(image_id),
            )
            inst_dict = {}
            ...
            )  # meta_data['viewport_size_x'].flatten()[0]
            height = (
                self.config.IMAGE_MIN_DIM
            )  # meta_data['viewport_size_y'].flatten()[0]
        ...
        self.source_image_ids[source] = np.arange(
            num_images_before_load, num_images_after_load
        )
        print(
    ...
    def load_real_scenes(self, dataset_dir):
        ...
        folder_list = [
            name for name in glob.glob(image_dir + "/*") if os.path.isdir(name)
        ]
        print(folder_list)
    ...
    def load_coco(self, dataset_dir, subset, class_names):
        ...
        image_dir = os.path.join(
            dataset_dir, "images", "train2017" if subset == "train" else "val2017"
        )
        # print('************')
        # print(image_dir)
        # Create COCO object
        ...
        for i, image_id in enumerate(image_ids):
            self.add_image(
                ...
                annotations=coco.loadAnns(
                    coco.getAnnIds(imgIds=[image_id], iscrowd=False)
                ),
            )
    ...
    def load_objs(self, image_id, is_normalized):
        ...
        for i, line in enumerate(lines):
            ...
            if len(words) == 3:  ## real data
                if words[2][-3:] == "npz":
                    ...
                    mesh_file = os.path.join(
                        self.config.OBJ_MODEL_DIR, "real_val", obj_name
                    )
                else:
                    mesh_file = os.path.join(
                        self.config.OBJ_MODEL_DIR,
                        "real_" + self.subset,
                        words[2] + ".obj",
                    )
                flip_flag = False
            ...
            vertices, faces = utilscopy.load_mesh(mesh_file, is_normalized, flip_flag)
            Vs.append(vertices)
            Fs.append(faces)
    ...
    def process_data(self, mask_im, coord_map, inst_dict, meta_path, load_RT=False):
        ...
        cv2.imwrite("clipbefore.jpg", coord_map.astype(int))
        coord_map = np.array(coord_map, dtype=np.float32) / 255
        coord_map[:, :, 2] = 1 - coord_map[:, :, 2]
        cv2.imwrite("clipafter.jpg", (coord_map * 255).astype(int))
            ...
            if len(words) == 3:
                ...
                if words[2][-3:] == "npz":
                    npz_path = os.path.join(
                        self.config.OBJ_MODEL_DIR, "real_val", words[2]
                    )
    ...
    def load_mask(self, image_id):
        ...
        if info["source"] in ["CAMERA", "Real"]:
            ...
            masks, coords, class_ids, scales = self.process_data(
                mask_im, coord_map, inst_dict, meta_path
            )
            ...
            for annotation in annotations:
                ...
                if class_id:
                    m = utilscopy.annToMask(annotation, info["height"], info["width"])
    ...
    def load_augment_data(self, image_id):
        ...
        if info["source"] in ["CAMERA", "Real"]:
            ...
            image, masks = utilscopy.rotate_and_crop_images(
                image, masks=masks, coords=None, rotate_degree=rotate_degree
            )
