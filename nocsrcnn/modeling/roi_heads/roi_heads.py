...
class NOCSRCNNROIHeads(StandardROIHeads):
    ...
    def argmax_and_reshape(self, mrcnn_coord_bin):
        ...
        mrcnn_coord_bin_reshape = mrcnn_coord_bin.contiguous().view(
            -1, mrcnn_coord_shape[-1]
        )  # [702464, 32]
        mrcnn_coord_bin_value = torch.argmax(
            mrcnn_coord_bin_reshape, axis=-1
        ).float() / float(self.num_bins)
        mrcnn_coord_bin_value = mrcnn_coord_bin_value.view(
            mrcnn_coord_shape[:-1]
        )  # [128, 28, 28, 6]
    ...
    def _forward_nocs(self, features, instances):
        ...
        if self.training:
            ...
            target_coords_x, target_coords_y, target_coords_z = (
                target_coords[:, :, :, 0],
                target_coords[:, :, :, 1],
                target_coords[:, :, :, 2],
            )
            ...
            if domain_ix[0] == 0:
                target_class_ids = torch.ones(target_class_ids.shape) * (
                    self.cfg.MODEL.ROI_NOCS_HEAD.NUM_CLASSES
                )
            ...
            if self.COORD_USE_BINS:
                mrcnn_coord_x_bin, mrcnn_coord_x_feature = self.nocs_head_x(
                    nocs_features
                )  # mrcnn_coord_x_bin   ([128, 28, 28, 7, 32])
                mrcnn_coord_y_bin, mrcnn_coord_y_feature = self.nocs_head_y(
                    nocs_features
                )
                mrcnn_coord_z_bin, mrcnn_coord_z_feature = self.nocs_head_z(
                    nocs_features
                )
                ...
                if self.USE_SYMMETRY_LOSS:
                    coord_loss = mrcnn_coord_bins_symmetry_loss_graph(
                        target_mask,
                        target_coords,
                        target_class_ids,
                        target_domain_labels,
                ...
                if self.COORD_SHARE_WEIGHTS:
                    ...
                    mrcnn_coord_x, mrcnn_coord_x_feature = self.nocs_head_x(
                        nocs_features
                    )
                    mrcnn_coord_y, mrcnn_coord_y_feature = self.nocs_head_y(
                        nocs_features
                    )
                    mrcnn_coord_z, mrcnn_coord_z_feature = self.nocs_head_z(
                        nocs_features
                    )
                    mrcnn_coords = torch.stack(
                        (mrcnn_coord_x, mrcnn_coord_y, mrcnn_coord_z), dim=-1
                    )
                if self.USE_SYMMETRY_LOSS:
                    """
                    ...
                    coord_loss = mrcnn_coords_symmetry_loss_graph_3(
                        target_mask,
                        target_coords,
                        target_class_ids,
                        target_domain_labels,
