...
def mrcnn_coord_bins_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    y_true_bins_stack = clip_by_tensor(y_true_bins_stack,0,num_bins-1).long() #[num_pos_rois, 28, 28, 6, 3]
...
class NOCSRCNNGraphConvSubdHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.conv_norm_relus:
            # print("*****",x.shape,"======")
            x = layer(x)
        x_feature = F.relu(self.deconv(x))  #(batch_size*batch_per_image(64)) * 256 * 28 * 28     [128, 256, 28, 28]
