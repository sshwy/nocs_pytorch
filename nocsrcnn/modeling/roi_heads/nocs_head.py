...
def mrcnn_coord_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    pos_class_rotation_theta = pos_class_ids.map_(pos_class_ids, class_id_to_theta) #旋转的角度 [128]
    ...
    pos_class_rotation_matrix = pos_class_rotation_matrix.view(-1, 1, 1, 3, 3)# [num_pos_rois, 1, 1, 3, 3]
    ...
    y_true = torch.gather(target_coords, 0, pos_ix_)  ## shape: [num_pos_rois, height, width, 3]
    ...
    y_true_stack = torch.cat((y_true, rotated_y_true_1, rotated_y_true_2, rotated_y_true_3,
                    rotated_y_true_4, rotated_y_true_5),dim=4) ## shape: [num_pos_rois, height, width, 3, 6]
    y_true_stack = y_true_stack.permute(0, 1, 2, 4, 3)  ## shape: [num_pos_rois, height, width, 6, 3]
    y_true_stack = y_true_stack + 0.5

    y_pred = pred_coords[pos_ix.long(), pos_class_ids.long(),: ,: ,:  ]#pred_coords[indices]
    y_pred = y_pred.unsqueeze(3) 
    y_pred_stack = y_pred.repeat(1, 1, 1, y_true_stack.shape[3], 1)

    diff = torch.abs(y_true_stack - y_pred_stack)  ## shape: [num_pos_rois, height, width, 6, 3]
    ...
    arg_min_rotation = torch.argmin(total_sum_loss_in_mask, dim=-1).long()  ##shape: [num_pos_rois]

    index_1 = torch.arange(0,arg_min_rotation.shape[0]).cuda().long()
    min_loss_in_mask = sum_loss_in_mask[index_1, arg_min_rotation, :]  ## shape: [num_pos_rois, 3]

    num_of_pixels = num_of_pixels.unsqueeze(1)
    mean_loss_in_mask = min_loss_in_mask.div(num_of_pixels.expand_as(min_loss_in_mask))  ## shape: [num_pos_rois, 3]
...
def mrcnn_coord_symmetry_euclidean_distance_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    mean_squared_diff_sum_in_mask = min_squared_diff_sum_in_mask.div(num_of_pixels)  ## shape: [num_pos_rois]
...
def mrcnn_coord_bins_symmetry_loss_graph_buyongle(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coord_bins_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    num_of_pixels = mask.sum(dim = [1, 2]) + 0.00001  ## shape: [num_pos_rois]
    #print(num_of_pixels)
    cross_loss_in_mask = cross_loss.mul(reshape_mask)  ## shape: [num_pos_rois, height, width, 6, 3]
    sum_loss_in_mask = cross_loss_in_mask.sum(dim=[1, 2])  ## shape: [num_pos_rois, 6, 3]
    total_sum_loss_in_mask = sum_loss_in_mask.sum(dim=-1)  ## shape: [num_pos_rois, 6]
...
def mrcnn_coords_reg_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coord_reg_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    y_true = torch.gather(target_coords, 0, pos_ix_)  ## shape: [num_pos_rois, height, width]
...
def mrcnn_coord_l1_loss_graph_1(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coord_l2_loss_graph_1(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coord_smoothl1_loss_graph_1(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coords_l2_loss_graph_1(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
...
def mrcnn_coords_symmetry_loss_graph_3(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords,loss_fn):
...
def mrcnn_coords_symmetry_smoothl1_loss_graph_3(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    y_pred_stack = y_pred.repeat(1, 1, 1, y_true_stack.shape[3],  1)
    loss1 = torch.abs (torch.sub(input=y_true_stack, alpha=1, other=y_pred_stack))# [ roi ,28 ,28 ,6 ,3]
    loss2 = loss1-0.05
    loss3 = torch.pow(torch.abs (torch.sub(input=y_true_stack, alpha=1, other=y_pred_stack)),2)*5
...
def mrcnn_coords_symmetry_l1_loss_graph_3(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    y_pred_stack = y_pred.repeat(1, 1, 1, y_true_stack.shape[3],  1)
    loss = torch.abs (torch.sub(input=y_true_stack, alpha=1, other=y_pred_stack))# [ roi ,28 ,28 ,6 ,3]
    mask = target_masks[pos_ix]  ## shape: [num_pos_rois, height, width]
    num_of_pixels = mask.sum(dim = [1, 2]) + 0.00001  ## shape: [num_pos_rois]
    #print(num_of_pixels )
    reshape_mask = mask.view(mask.shape[0], mask.shape[1], mask.shape[2],  1, 1)
...
def mrcnn_coords_symmetry_l2_loss_graph_3(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    y_pred_stack = y_pred.repeat(1, 1, 1, y_true_stack.shape[3],  1)
    loss = torch.pow(torch.abs (torch.sub(input=y_true_stack, alpha=1, other=y_pred_stack)),2)# [ roi ,28 ,28 ,6 ,3]
...
def mrcnn_coords_symmetry_l2_loss_graph_1(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords):
    ...
    print('x: ',round(y_pred[:,:,:,0].max().item(),4),round(y_pred[:,:,:,0].min().item(),4),
            round(y_true[:,:,:,0].max().item(),4),round(y_true[:,:,:,0].min().item(),4) ,
        '  y: ',round(y_pred[:,:,:,1].max().item(),4),round(y_pred[:,:,:,1].min().item(),4),
            round(y_true[:,:,:,1].max().item(),4),round(y_true[:,:,:,1].min().item(),4),
        '  z: ',round(y_pred[:,:,:,2].max().item(),4),round(y_pred[:,:,:,2].min().item(),4),
            round(y_true[:,:,:,2].max().item(),4),round(y_true[:,:,:,2].min().item(),4))
    y_true = y_true - 0.5
    y_true = y_true.unsqueeze(4)  ## shape: [num_pos_rois, height, width, 3, 1]
...
class NOCSRCNNGraphConvSubdHead(nn.Module):
    ...
    def build_bin_coord_graph(self ,x):
        ...
        x_feature = F.relu(x)  #(batch_size*batch_per_image(64)) * 256 * 28 * 28     [128, 256, 28, 28]
        x = self.conv_bins(x_feature)       #(batch_size*batch_per_image(64)) * (class_num*bin_num) * 28 * 28   [128, 224, 28, 28]
    ...
    def build_regress_coord_graph(self ,x):
        ...
        x_feature = F.relu(x)  #(batch_size*batch_per_image(64)) * 256 * 28 * 28     [128, 256, 28, 28]
        x = self.conv_reg(x_feature)       #(batch_size*batch_per_image(64)) * class_num * 28 * 28   [128, 224, 28, 28]
    ...
    def build_regress_coords_graph(self ,x):
        ...
        x_feature = F.relu(x)  #(batch_size*batch_per_image(64)) * 256 * 28 * 28     [128, 256, 28, 28]
        x = self.conv_reg3(x_feature)       #(batch_size*batch_per_image(64)) * (class_num*3) * 28 * 28   [128, 224, 28, 28]
        #print(self.conv_bins.weight[0].max())
